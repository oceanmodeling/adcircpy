...
main() {
  SECONDS=0
  run_coldstart_phase
  if grep -Rq "ERROR: Elevation.gt.ErrorElev, ADCIRC stopping." punswan.log; then
    duration=$SECONDS
...
run_coldstart_phase() {
  ...
  mpiexec -n 2 punswan 2>&1 | tee ../punswan.log
  clean_directory
  cd ..
}
...
run_hotstart_phase() {
  ...
  adcprep --np 2 --partmesh
  adcprep --np 2 --prepall
  mpiexec -n 2 punswan 2>&1 | tee -a ../punswan.log
  clean_directory
  cd ..
}
