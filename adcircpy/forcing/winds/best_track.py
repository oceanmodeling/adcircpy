...
class BestTrackForcing(WindForcing):
    ...
    def clip_to_bbox(self, bbox, bbox_crs):
        ...
        for _datetime in unique_dates:
            ...
            utm_crs = CRS(
                    ...
                    ellps={'GRS 1980': 'GRS80', 'WGS 84': 'WGS84'}[
    ...
    def plot_track(self, axes=None, show=False, color='k', **kwargs):
        ...
        for i in range(len(self.speed)):
            ...
            axes.quiver(self.longitude.iloc[i], self.latitude.iloc[i], U, V,
                        **kwargs)
            if i % 6 == 0:
                axes.annotate(
        ...
        for i, _datetime in enumerate(unique_datetimes):
            ...
            for idx in indexes:
                if indexes[-1] + 1 < len(data['datetime']):
                    ...
                    vx = np.copysign(
                            dx / dt, data['longitude'][indexes[-1] + 1] -
                            data['longitude'][idx],
                    )
                    vy = np.copysign(
                            dy / dt, data['latitude'][indexes[-1] + 1] -
                            data['latitude'][idx],
                    )
                else:
                    dt = (
                    ...
                    vx = np.copysign(
                            dx / dt,
                            data['longitude'][idx] - data['longitude'][
                                indexes[0] - 1],
                    )
                    vy = np.copysign(
    ...
    @classmethod
    def from_fort22(cls, fort22: PathLike,
                    nws: int = None, ) -> 'BestTrackForcing':
        if nws is None:
            nws = 20
