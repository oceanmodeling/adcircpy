...
class AdcircCommand:
    ...
    def run(self):
        ...
        if self._args.generate_only:
            self.driver.write(self._args.output_directory,
    ...
    def server_config(self):
        if self._args.hostname:
            if not self._args.use_slurm or not self._args.use_torque or not self._args.use_pbs:
    ...
    def _enable_output(self, driver, name, _type):
        ...
        getattr(driver, f'set_{name}_{_type}_output')(
            sampling_rate=fs, harmonic_analysis=ha, spinup=fss,
    ...
    @lru_cache(maxsize=None)
    def _mesh(self):
        mesh = AdcircMesh.open(self._args.mesh, self._args.crs)
