...
def generate_only(parser):
    ...
    parser.add_argument(
            '--generate-only', '--no-run', '--skip-run', action='store_true',
            help=msg
    )


def log_level(parser):
    parser.add_argument('--log-level', choices=['info', 'warning', 'debug'],
                        default='warning')
...
def server(parser):
    ...
    parser.add_argument('--wdir',
                        required=True if '--hostname' in sys.argv else False)
    parser.add_argument('--keep-wdir', action='store_true')
    parser.add_argument('--binaries-path', '--binaries-prefix',
    ...
    parser.add_argument('--module', default=list(), action='append',
                        dest='modules')
...
def tidal_constituents(parser):
    ...
    parser.add_argument('--tidal-database', choices=['hamtide', 'tpxo'],
                        default='hamtide')
...
def boundaries_generation(parser):
    ...
    parser.add_argument('--land-ibtype', default=20, type=int,
                        choices=[0, 10, 20])
    parser.add_argument('--island-ibtype', default=21, type=int,
                        choices=[1, 11, 21])
...
def surface_output(physical_var, parser, spinup=False):
    ...
    if not spinup:  # disable harm for spinup, less clutter.
        ...
        parser.add_argument(
                long_name, short_name, action='store_true', default=False,
                help=msg
        )
