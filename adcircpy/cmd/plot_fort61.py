...
def parse_args():
    ...
    parser.add_argument('path',
                        help='Path to ADCIRC fort.61 or fort.61.nc file.')
    parser.add_argument(
        'vertical_datum',
    ...
    parser.add_argument('--coops-only', action='store_true',
...
def main():
    ...
    for station_id, data in fort61:
        ...
        if args.coops_only and not np.all(np.isnan(coops.values)):
            plt.plot(coops.datetime, coops.values, label='COOPS', color='b',
                     linewidth=0.7)
            plt.plot(fort61.datetime, data['values'], label='ADCIRC',
                     color='r', linewidth=0.7)

        else:
            if not args.coops_only:
                plt.plot(
                    fort61.datetime, data['values'], label='ADCIRC', color='r',
                    linewidth=0.7
                )
        fig = plt.gcf()
        if fig.get_axes():
