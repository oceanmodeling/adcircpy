...
class AdcircMesh(metaclass=AdcircMeshMeta):
    ...
    def generate_linear_mannings_n(
        ...
        min_depth = np.min(self.values) if min_depth is None else float(
            min_depth)
        max_depth = np.max(self.values) if max_depth is None else float(
            max_depth)

        values = min_value + (self.values - min_depth) * (
                    max_value - min_value) / (
                         max_depth - min_depth
                 )
