...
class NodalAttributes:
    ...
    def add_attribute(self, name: str, units: str = None):
        if name in self._attributes:
            raise AttributeError(
                f"Cannot add nodal attribute with name "
    ...
    def add_patch(self, name, patch, value):
        ...
        idxs = [
            row.Index
            for row in self._fort14.nodes.gdf.geometry.within(patch).itertuples()
        ]
    ...
    with open(path, "r") as f:
        ...
        for i in range(NAttr):
            ...
            while j < numOfNodes:
                ...
                j += 1
            values[np.where(np.isnan(values[:, 0])), :] = fort13[attribute_name][
                "defaults"
            ]
