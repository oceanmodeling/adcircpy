...
class NodalAttributes:
    ...
    def set_attribute(self, name, values, coldstart: bool = False,
                      hotstart: bool = False):
        if name not in self.get_attribute_names():
            raise AttributeError(
    ...
    def add_patch(self, name, patch, value):
        ...
        idxs = [
            ...
            self._fort14.nodes.gdf.geometry.within(patch).itertuples()
        ]
        ...
        for attribute, data in fort13.items():
            ...
            full_values = np.full(
                    (self._fort14.values.size,
                     np.asarray(data['defaults']).flatten().size), np.nan
            )
