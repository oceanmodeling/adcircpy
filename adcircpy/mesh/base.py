...
class Nodes:
        ...
        if crs is not None:
            ...
            if not crs.equals(self.crs):
                transformer = Transformer.from_crs(self.crs, crs,
                                                   always_xy=True)
                x, y = transformer.transform(self.coord[:, 0],
                                             self.coord[:, 1])
    ...
    def gdf(self):
        if not hasattr(self, '_gdf'):
            ...
            for id, coord, values in zip(self._id, self._coords, self.values):
                data.append(
                        {'geometry': Point(coord), 'id': id, 'values': values})
    ...
    def get_index_by_id(self, id: Hashable):
        if not hasattr(self, 'node_id_to_index'):
            self.node_id_to_index = {self.id[i]: i for i in
                                     range(len(self.id))}
        return self.node_id_to_index[id]

    def get_id_by_index(self, index: int):
        if not hasattr(self, 'node_index_to_id'):
            self.node_index_to_id = {i: self.id[i] for i in
                                     range(len(self.id))}
        return self.node_index_to_id[index]

    def to_dict(self):
        nodes = {nid: (coo, val) for nid, coo, val in
                 zip(self._id, self._coords, self.values)}
        return nodes
...
class Elements:
    ...
    def get_index_by_id(self, id: Hashable):
        if not hasattr(self, 'element_id_to_index'):
            self.element_id_to_index = {self.id[i]: i for i in
                                        range(len(self.id))}
        return self.element_id_to_index[id]

    def get_id_by_index(self, index: int):
        if not hasattr(self, 'element_index_to_id'):
            self.element_index_to_id = {i: self.id[i] for i in
                                        range(len(self.id))}
    ...
    def get_ball(self, order: int, id=None, index=None):
        ...
        if not order >= 0:
            raise TypeError(
                "Argument 'order' must be of greater " 'than zero.')
        ...
        for i in range(order):
            ...
            new_neighbors = set(
                    [item for sublist in new_neighbors for item in sublist])
            eidxs = eidxs.union(
                    set(
                            np.where(
                                    np.logical_and(
                                            np.any(np.isin(self.array, list(
                                                set(new_neighbors))), axis=1),
                                            np.any(
                                                np.isin(self.array, elements),
                                                axis=1),
                                    )
                            )[0]
                    )
...
class Hull:
            ...
            data.append(
                    {
                        'geometry': Polygon(
                                ...
                                [row.geometry.coords for _, row in
                                 interiors.iterrows()],
                        ),
                        'bnd_id': bnd_id,
                    }
    ...
    def implode(self) -> gpd.GeoDataFrame:
        return gpd.GeoDataFrame(
                {
                    'geometry': MultiPolygon([polygon.geometry for polygon in
                                              self().itertuples()])},
                crs=self._grd.crs,
        )
            ...
            sms2dm.write(
                    {
                        'ND': {
                            ...
                            + 1: (
                                coord,
                                -self.values[i] if not np.isnan(
                                        self.values[i]) else -99999,
                            )
                            for i, coord in enumerate(self.coords)
                        },
                        'E3T': {i + 1: index + 1 for i, index in
                                enumerate(self.triangles)},
                        'E4Q': {i + 1: index + 1 for i, index in
                                enumerate(self.quads)},
                    },
                    path,
                    overwrite,
            )
