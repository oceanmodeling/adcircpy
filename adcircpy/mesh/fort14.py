        ...
        if not hasattr(self, '_indexes'):
            ...
            for data in self._data.values():
                self._indexes.append(
                        list(map(self._mesh.nodes.get_index_by_id,
                                 data['node_id']))
                )
...
class Fort14Boundaries:
    ...
    def land(self):
        if not hasattr(self, '_land'):
            self._land = LandBoundaries(self._mesh,
    ...
    def interior(self):
        if not hasattr(self, '_interior'):
            self._interior = InteriorBoundaries(self._mesh,
    ...
    def inflow(self):
        if not hasattr(self, '_inflow'):
            self._inflow = InflowBoundaries(self._mesh,
    ...
    def outflow(self):
        if not hasattr(self, '_outflow'):
            self._outflow = OutflowBoundaries(self._mesh,
    ...
    def weir(self):
        if not hasattr(self, '_weir'):
            self._weir = WeirBoundaries(self._mesh,
    ...
    def culvert(self):
        if not hasattr(self, '_culvert'):
            self._culvert = CulvertBoundaries(self.mesh,
...
class Fort14(Grd):
    ...
    def open(cls, path, crs=None):
        ...
        _grd['nodes'] = {id: (coords, -val) for id, (coords, val) in
                         _grd['nodes'].items()}
        return cls(**_grd)
    ...
    def to_dict(self, boundaries=True):
        ...
        if boundaries is True:
            _grd.update(
                    {
                        'nodes': {
                            id: (coord, -val) for id, (coord, val) in
