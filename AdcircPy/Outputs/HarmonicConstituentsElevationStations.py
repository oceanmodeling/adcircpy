...
class HarmonicConstituentsElevationStations(OrderedDict):
  ...
  def __init__(self, **station_data):
    super(HarmonicConstituentsElevationStations, self).__init__(**station_data)
    ...
    with open(fort51, 'r') as f:
      ...
      for station_id in range(num_of_stations):
        ...
        for constituent in constituents:
          ...
          phase = float(line[1])
          stations[station_id]['data'][constituent] = {"orbital_frequency" : orbital_constants.orbital_frequency[constituent],
                                                       "amplitude"         : amplitude,
                                                       "phase"             : phase}
    return cls(**stations)
  ...
  def from_netcdf(cls, fort51):
    ...
    for i, station in enumerate(stations.keys()):
      ...
      for j, component in enumerate(nc.variables['const'][:]):
        stations[station]['data'][component.tostring().strip().decode('utf-8')]={
