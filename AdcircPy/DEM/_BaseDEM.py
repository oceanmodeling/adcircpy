...
class _BaseDEM(_Figure):
  ...
  def dump(self, path, driver='Gtiff'):
    ...
    ds = driver.Create(path, self.values.shape[1], self.values.shape[0], 1, gdal.GDT_Float32)
  ...
  def downsample_by_factor(self, dsfact):
    ...
    self._geoTransform = (self.geoTransform[0], res_x, self.geoTransform[2],
  ...
  def transform_to_epsg(self, epsg):
    ...
    self._geoTransform = ((self.x[0], (self.x[-1]-self.x[0])/(len(self.x)-1), 0, np.max(self.y), (np.min(self.y)-np.max(self.y))/(len(self.y)-1)))
    self._epsg = epsg
