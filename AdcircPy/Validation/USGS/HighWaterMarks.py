...
class HighWaterMarks(OrderedDict):
  ...
  def __init__(self, event_name, event_year, filter_dict=dict(), **station_data):
  ...
  @classmethod
  def from_csv(cls, eventName, eventYear, csvpath, filter_dict=None):
    filter_dict = cls._init_filter_dict(filter_dict)
    with open(csvpath, 'r') as f: 
      ...
      for i, line in enumerate(csvfile):
        ...
        for j, column in enumerate(headers):
          if column in ['longitude', 'latitude', 'elev_ft', 'site_latitude', 'site_longitude']:
  ...
  def _get_event_id_from_name(cls, event_name):
    ...
    for name, year, _id in events:
      events_dict[name.lower()+str(year)]={'name':name, 'year' : year, 'id':_id}
    if event_name.lower() in events_dict.keys():
      return events_dict[event_name.lower()]['name'], events_dict[event_name.lower()]['year'], events_dict[event_name.lower()]['id']
    else:
