...
class HighWaterMarkValidation(object):
  ...
  def _parse_args(self):
    parser = argparse.ArgumentParser(description='Program for validating tidal harmonic constituent outputs against COOPS data.')
    parser.add_argument('event_id', help='ATCF id of the hurricane')
    parser.add_argument('maxele', help='Path to the maxele file.')
    parser.add_argument('datum_grid', help='Path to the datum conversion grid from LMSL to NAVD88. Required since HWMs are in NAVD88 and usually the mesh is in LMSL based on VDatum.')
  ...
  def _init_maxele(self):
    if Maxele.is_netcdf(self.args.maxele):
      ...
      if self.args.fort14 is None:
        raise IOError("A mesh file is required if the maxele file is ASCII format.")
      self.maxele = Maxele.from_ascii(self.args.maxele,
                                       self.args.fort14,
                                       datum_grid=self.args.datum_grid)
    # convert to feet
  ...
  def _init_HighWaterMarks(self):
    ...
    if self.args.csv is not None:
      ...
      self.HighWaterMarks = HighWaterMarks.from_event_name(self.args.event_id)
  ...
  def _init_CDH(self):
    ...
    plt.title('Count of dry forecast points for {}, {}'.format(self.HighWaterMarks.event_name, self.HighWaterMarks.event_year))
    plt.ylim([0,np.max(CDH)+2])
    plt.xlabel('Observed Water Level Threshold (ft NAVD88)')
  ...
  def _init_POD_and_FAR(self):
    ...
    for Thres in self.wl_thresholds:
      ...
      if (Obs[hit].size+np.float(Obs[miss].size))>0:
        self._POD.append(np.float(Obs[hit].size)/(np.float(Obs[hit].size)+np.float(Obs[miss].size)))
      else:
        self._POD.append(np.nan)
      if (np.float(Obs[fa].size)+np.float(Obs[hit].size))>0:
        self._FAR.append(np.float(Obs[fa].size)/(np.float(Obs[fa].size)+np.float(Obs[hit].size)))
      else:
    ...
    plt.plot(self.wl_thresholds, self.wl_thresholds*[0], color='black', linestyle='--')
    for i, count in enumerate(self._count):
      plt.annotate(count, (self.wl_thresholds[i], 0), verticalalignment='top', horizontalalignment='center')
    plt.ylim([-0.2,1.2])
    plt.yticks(np.arange(0, 1.2, step=0.2))
    plt.legend(loc='best')
    plt.title('Probability of detection for {}, {}'.format(self.HighWaterMarks.event_name, self.HighWaterMarks.event_year))
    plt.xlabel('Water Level Threshold (ft NAVD88)')
    plt.ylabel('Skill')
  ...
  def _init_statistics(self):
    ...
    self._statistics['pearson_r'] = self.__get_pearson_r(self.dataframe["Observation"])
    self._statistics['bias'] = self.__get_bias(self.dataframe["Observation"])
    self._statistics['RMS_demeaned'] = self.__get_RMS_demeaned(self.dataframe["Observation"], self._statistics['bias'])
    self._statistics['scatter_index'] = self.__get_scatter_index(self.dataframe["Observation"], self._statistics['RMS_demeaned'])
    self._statistics['90percent_index'] = self.__get_90percent_index(self.dataframe["Observation"])

  def _init_best_case_statistics(self):
    self._best_case_statistics=dict()
    self._best_case_statistics['pearson_r'] = self.__get_pearson_r(self.dataframe["best_case"])
    self._best_case_statistics['bias'] = self.__get_bias(self.dataframe["best_case"])
    self._best_case_statistics['RMS_demeaned'] = self.__get_RMS_demeaned(self.dataframe["best_case"], self._best_case_statistics['bias'])
    self._best_case_statistics['scatter_index'] = self.__get_scatter_index(self.dataframe["best_case"], self._best_case_statistics['RMS_demeaned'])
    self._best_case_statistics['90percent_index'] = self.__get_90percent_index(self.dataframe["best_case"])
  ...
  def __init_nanmean(self):
    ...
    self.missed = list()
    values = self.__init_values_dict()
    for station in self.HighWaterMarks.keys():
      x = self.HighWaterMarks[station]['longitude']
      y = self.HighWaterMarks[station]['latitude']
      element = self.maxele.get_element_containing_coord((x,y))
      if element is None:
        ...
        with warnings.catch_warnings():
          ...
          value = np.nanmean(np.ma.filled(self.maxele.values[element], fill_value=np.nan))
    ...
    for station in self.HighWaterMarks.keys():
      ...
      if element is None:
        ...
        if self.maxele.values[element][indexes].mask.all():
          ...
          value = np.divide(_values, _distances).sum()/(1./np.asarray(_distances)).sum()
          self._values["Observation"].append(self.HighWaterMarks[station]["elev_ft"])
          self._values["ADCIRC"].append(value)
          self._values["Quality_ID"].append(self.HighWaterMarks[station]["hwmQualityName"])
          self._values["station_id"].append(station)
          self.__init_uncertainties()
    self.dataframe = pd.DataFrame(self._values)
  ...
  def __init_best_case(self, station, uncertainty, value):
    ...
    if (cases[0]< value < cases[1])==False:
  ...
  def __init_regression_plot(self):
    ...
    plt.errorbar(x, y, xerr=self.dataframe["Uncertainty"], fmt='none', color='darkorange')
    plt.gca().set(xlabel='Observations [feet:NAVD88]', ylabel='Hindcast [feet:NAVD88]')
    plt.title('High Water Mark Validation for {}, {}'.format(self.HighWaterMarks.event_name, self.HighWaterMarks.event_year))
    plt.axis('scaled')
  ...
  def __init_regression_plot_best_case(self):
    ...
    sns.regplot(x, y, fit_reg=False, ci=None)
    plt.gca().set(xlabel='Observations [feet:NAVD88]', ylabel='Hindcast [feet:NAVD88]')
    plt.title('High Water Mark Validation for {}, {} (best case)'.format(self.HighWaterMarks.event_name, self.HighWaterMarks.event_year))
    plt.axis('scaled')
    plt.axis([0, 1.1*np.max([x.max(), y.max()]), 0, 1.1*np.max([x.max(), y.max()])])
    plt.gcf().set_size_inches(10.5, 10.5)
